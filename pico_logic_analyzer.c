...
int main() {
    ...
    logic_analyzer_init(pio, &logic_analyzer_sm, &logic_analyzer_offset, CAPTURE_GPIO_PIN_BASE);
    int nec_8_sm = nec_8_init(pio, CAPTURE_GPIO_PIN_BASE);
    ...
    while (true)
    {
        ...
        if (!pio_sm_is_rx_fifo_empty(pio, nec_8_sm))
        {
            uint32_t rx_frame = pio_sm_get(pio, nec_8_sm);
        ...
        if ('q' == c)
        {
            if (pio_sm_is_rx_fifo_empty(pio, logic_analyzer_sm))
            {
                printf("No data in logic analyzer FIFO\n");
            }
            else
            {
                uint32_t data = pio_sm_get(pio, logic_analyzer_sm);
                printf("0x%08x\n", data);
            }
        }
        else if ('r' == c)
        {
            logic_analyzer_restart(pio, logic_analyzer_sm, logic_analyzer_offset);
        }
        else if ('c' == c)
        {
